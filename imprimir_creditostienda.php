...
//$pdf->Image('./files/imagenes/logohlv4.png',10,2,55,57,'png');
//$pdf->Image('./files/imagenes/logov4.png',5,2,70,57,'png');
//$pdf->Image('./files/imagenes/logov6.png',20,14,40,40,'png');
 $pdf->Cell(30,10,"TICKET ",0,1,"L");
 $pdf->SetFont('Helvetica','B',10);  
 $pdf->setX(5);
//  $pdf->Cell(30,5,"Direccion 3er anillo entre roca coronado \n ",0,1,"L");
//  $pdf->Cell(30,5,"y centenario calle asusaqui",0,1,"L");
 $pdf->setX(5);
//   $pdf->Cell(30,5,"Whatsapp: 75630095",0,1,"L");


$pdf->SetFont('Helvetica','B',12);    
$pdf->setY(35);
$pdf->setX(5);
//$pdf->Cell(10,8,utf8_decode("N° ".$idventa),1,1,'C');
// $textypos+=10;
//$pdf->setX(5);
...
//$pdf->Cell(5,5,"Av. Grigota Esquina Muchiri",0,1,'C');
$pdf->setX(35);
//$pdf->Cell(5,5,"Comercial La Ramada 2do piso",0,1,'C');
$pdf->setX(35);
//$pdf->Cell(5,5,"Tienda #364",0,1,'C');
//$pdf->setX(5);
$pdf->SetFont('Helvetica','B',12);
$pdf->setX(5);
$pdf->Cell(5,5,"Fecha:".date("d/m/Y", strtotime($fecha)).' '.$hora,0,1,'L');
//$pdf->setX(5);
//$pdf->Cell(5,5,"Hora:".$hora,0,1,'L');
$pdf->setX(5);
...
$pdf->setX(2);
$pdf->Cell(5,5,'  FECHA.         HORA          VENDEDOR       MONTO PAGADO');
$pdf->Ln(5);

$pdf->setX(2);
$pdf->Cell(5,5,'--------------------------------------------------------------------------------------------------------',0,0,"L");
$sentencia = $base_de_datos->query("SELECT productos.nombre,productos_vendidos.precio as precioVenta,productos_vendidos.cantidad,productos_vendidos.descuento FROM productos_vendidos ,productos ,ventastienda where ventastienda.id='$idventa' and ventastienda.id=productos_vendidos.id_venta and productos_vendidos.id_producto=productos.id");
$productos = $sentencia->fetchAll(PDO::FETCH_OBJ);
...
foreach($resultado_sql_abonos as $abono){
   ...
   $pdf->Cell(5,5,date("d/m/Y", strtotime($abono->fecha)).' '.$abono->hora,0,0,"L");
   $pdf->setX(30);
