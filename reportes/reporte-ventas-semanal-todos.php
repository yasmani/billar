...
//Activamos el almacenamiento en el buffer
  ...
  include 'plantilla_de_todas_las_vendas.php';
	...
	$sentencia = $base_de_datos->query(" SELECT ventastienda.tipoDeVenta as tipo_venta , ventastienda.hora,ventastienda.descuento,ventastienda.fecha,productos.precioCompra, ventastienda.id,productos.codigo,productos.nombre as nombre_producto ,productos_vendidos_tienda.cantidad,productos_vendidos_tienda.precio,usuario.usuario FROM usuario, `ventastienda` , productos_vendidos_tienda , productos WHERE ventastienda.id=productos_vendidos_tienda.id_venta and productos_vendidos_tienda.id_producto=productos.id and usuario.id=ventastienda.idUsuario and ventastienda.fecha BETWEEN '$inicio' and '$fin' ORDER BY `ventastienda`.`id` DESC;
	;");
	$sql = $base_de_datos->query("SELECT sum(total) as total, sum(descuento)as descuento from  ventastienda  where  ventastienda.fecha BETWEEN '$inicio' AND '$fin ' ;");
