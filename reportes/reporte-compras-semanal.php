...
//Activamos el almacenamiento en el buffer
  include 'plantilla_de_todas_las_compras.php';
	...
	$sentencia = $base_de_datos->query("SELECT codigo , productos.nombre,productos.descripcion,SUM(cantidad) as cantidad,sum(productos.precioCompra) as monto,productos.precioVenta as precio,productos.precioCompra as precioCompra from productos_comprados_tienda,productos  WHERE productos_comprados_tienda.id_producto=productos.id and  productos_comprados_tienda.id_compra IN (SELECT id FROM `comprastienda` WHERE fecha BETWEEN '$inicio' AND '$fin')GROUP by  productos_comprados_tienda.id_producto");
 ...
 $cant=0;
	foreach($ventas as $venta){ 
		...
		$pdf->Cell(30,6,$venta->codigo,1,0,'L',0);
		$pdf->SetFont('Arial','B',8); 
		$pdf->Cell(60,6,utf8_decode($venta->nombre),1,0,'L',0);
