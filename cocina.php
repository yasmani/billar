...
//header("Refresh: 5; URL='cocina.php'");
...
$sql = $base_de_datos->query("SELECT productos.codigo,productos.nombre ,sum(productos_vendidos_tienda.cantidad) as cantidad FROM `ventastienda` ,productos ,productos_vendidos_tienda WHERE ventastienda.id=productos_vendidos_tienda.id_venta and productos_vendidos_tienda.id_producto=productos.id and ventastienda.arqueo=0 and ventastienda.entregado=0 GROUP by codigo");
$platos = $sql->fetchAll(PDO::FETCH_OBJ);
?>
	...
	<div class="col-xs-12">
		...
		foreach($platos as $platos){ 
		   ...
		   echo ' <th style="color:black;background-color:yellow;font-size:20px">';
		     echo '<p style="color:red;font-size:25 px">'.number_format($platos->cantidad)."</p>"." ".$platos->nombre;
		     //echo '<br>';
		    echo '</th>';
		    if($i%5==0){
		        echo '<tr></tr>';
       ...
       name="meeting-time"  
		...
		<table style="color:white" name="tabla" class="table table-bordered">
			...
			<tbody>
				...
				<td  ><?php echo $venta->entregado==0? "<p style='color:black;background-color:yellow'>PENDIENTE </p>":"<p style='color:white;background-color:green '>ENTREGADO</p>" ?></td>
