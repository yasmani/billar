...
foreach ($_SESSION["carrito"] as $producto) {
	...
	$sentenciaExistencia->execute([$producto->precioCompra,$producto->precioVenta,$producto->cantidad, $producto->id]);
			...
			$printer->text("-----------------------------" . "\n");
			$printer->text("TIENDA 'BETY'.\n");
			$printer->text(date("d-m-Y H:i:s") . "\n");
			// $printer->text("Celular: 76319897"."\n");
			$printer->text("Direccion:".'SAGRADO CORAZON'."\n");
			$printer->text("--------------------------------------------" . "\n");
			$printer->setJustification(Printer::JUSTIFY_CENTER);
			...
			foreach ($_SESSION["carrito"] as $indice => $producto) {
				array_push($productos,new Producto($producto->descripcion, $producto->precioCompra, $producto->cantidad,$producto->total));
			}
	$total = 0;
