...
Class Egresos{
  ...
  //constructor
	...
	{
		$sql="INSERT INTO egresosdes(nombre,cantidad,detalle,total,fecha,condicion)
	...
	{
		$sql="UPDATE egresosdes SET egresoid='$egresoid',nombre='$nombre',cantidad='$cantidad',detalle='$detalle',total='$total'
		,fecha='$fecha' 
	...
	{
		$sql="UPDATE egresosdes SET condicion='0' WHERE egresoid='$egresoid'";
		return ejecutarConsulta($sql); //1 o 0
	}
	public function activar ($egresoid)
	{
		$sql="UPDATE egresosdes SET condicion='1' WHERE egresoid='$egresoid'";
		return ejecutarConsulta($sql); 
	}
  //muestra un tupla 
	public function mostrar($egresoid)
	{
		...
		$sql="SELECT * FROM egresosdes WHERE egresoid='$egresoid'";
	...
	{
		$fecha=date('Y-m-d');
		$sql="SELECT * FROM egresosdes where fecha='$fecha' and condicion='1'";
		return ejecutarConsulta($sql); //1 o 0
	}
	public function getEgreso($egresoid)//mostrar todos los registros
	{
		$fecha=date('Y-m-d');
		$sql="SELECT * FROM egresosdes where fecha='$fecha' and egresoid='$egresoid' and condicion='1'";
		return ejecutarConsulta($sql); //1 o 0
	}
	public function listar_fecha($inicio,$fin)//mostrar todos los registros
	{
		
		$sql="SELECT * FROM egresosdes where condicion='0' and fecha between '$inicio' AND '$fin' ORDER BY fecha asc";
		return ejecutarConsulta($sql); //1 o 0
	}
	  public function select()//mostrar todos los registros
	{
		$sql="SELECT *FROM egresosdes WHERE condicion=1";
		return ejecutarConsulta($sql); //1 o 0
	}
