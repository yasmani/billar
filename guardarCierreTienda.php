...
$sentencia = $base_de_datos->prepare("UPDATE apertura_tienda SET estado = ? , fecha_cierre=? WHERE id = ?;");
$resultado = $sentencia->execute(["1",$fecha_cierre,$id]);
...
////////////////////////////////////
$sentencia = $base_de_datos->query("SELECT codigo , productos.nombre,SUM(cantidad) as cantidad,sum(productos.precioVenta) as monto,productos.precioVenta as precio,productos.precioCompra as precioCompra from productos_vendidos_tienda,productos  WHERE productos_vendidos_tienda.id_producto=productos.id and  productos_vendidos_tienda.id_venta IN (SELECT id FROM `ventas` WHERE ventas.arqueo='0') GROUP by  productos_vendidos_tienda.id_producto");
...
// $printer->close();
///////////////////////////////////
$sentencia = $base_de_datos->query("SELECT * FROM ventastienda where arqueo='0';");
$productos = $sentencia->fetchAll(PDO::FETCH_OBJ);
  ...
  foreach($productos as $producto){ 
		...
		$id=$producto->id;
		$sentencia = $base_de_datos->prepare("UPDATE ventastienda SET arqueo = ?  WHERE id = ?;");
...
echo "cantidad";
echo $cantidad;
$sentencia = $base_de_datos->prepare("INSERT INTO arqueo_tienda(cantidad,monto,fecha) VALUES (?, ?,?);");
$fecha=date("Y-m-d ");
$resultado = $sentencia->execute(['1', '30',$fecha]);
 
//  guardar inventarios en  cierre 
$sentencia1 = $base_de_datos->query("SELECT * FROM productos WHERE lote!='BILLAR';");
$productos1 = $sentencia1->fetchAll(PDO::FETCH_OBJ);
$sentencia33 = $base_de_datos->prepare("INSERT INTO cierre(idcierre,idproducto,cantidad) VALUES (?, ?,?);");
  foreach($productos1 as $producto){ 
	 
	 
		 
		$resultado33 = $sentencia33->execute([$codigo_apertura, $producto->id,$producto->tienda]);
} 

// fin guardar inventarios
