...
if (isset($_GET['ticket'])) {
  ...
  $sentencia = $base_de_datos->query("SELECT * FROM ventas , usuario where ventas.id='$id' and usuario.id=ventas.idUsuario;");
...
$pdf->setX(12);
$pdf->Cell(5,5,'--------------------------------------------------------------------------------------------------------',0,0,"L");
$sentencia = $base_de_datos->query("SELECT productos.nombre,productos_vendidos.precio as precioVenta,productos_vendidos.cantidad,productos_vendidos.descuento FROM productos_vendidos ,productos ,ventas where ventas.id='$idventa' and ventas.id=productos_vendidos.id_venta and productos_vendidos.id_producto=productos.id");
$productos = $sentencia->fetchAll(PDO::FETCH_OBJ);
