...
$sentencia = $base_de_datos->prepare("INSERT INTO compras(id,fecha, total,usuario,proveedor,hora,condicion) VALUES (?,?,?, ?,?,?,?);");
$sentencia->execute([null,$ahora, $total,null,$proveedor,$hora,'1']);
...
$sentencia = $base_de_datos->prepare("INSERT INTO productos_comprados(id_producto,precio, id_compra, cantidad) VALUES (?,?, ?, ?);");
$sentenciaExistencia = $base_de_datos->prepare("UPDATE productos SET precioCompra = ?, precioVenta = ?,existencia = existencia + ? WHERE id = ?;");
foreach ($_SESSION["carrito"] as $producto) {
	...
	$sentenciaExistencia->execute([$producto->precioCompra,$producto->precioVenta,$producto->cantidad, $producto->id]);
}
