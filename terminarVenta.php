...
$total = $_POST["monto_con_descuento"];
...
$codigo_apertura=0;
 ...
 /*fin*/
$sentencia = $base_de_datos->prepare("INSERT INTO ventas(descuento,fecha,hora, total,Thing,devolver,cliente,detalle,orden,apertura,tipoDeVenta,idUsuario,nombre_de_usuario,saldo) VALUES (?,?,?,?,?,?,?, ?,?,?,?,?,?,?);");
$sentencia->execute([$descuento,$ahora,$hora,$total,$Thing, $devolver,$cliente,$detalle,$numero,$codigo_apertura,$tipoDeVenta,$idUsuario,$nombre_de_usuario,0]);
...
$sentencia = $base_de_datos->prepare("INSERT INTO productos_vendidos(id_producto,precio, id_venta, cantidad,descuento) VALUES (?,?, ?, ?,?);");
$sentenciaExistencia = $base_de_datos->prepare("UPDATE productos SET existencia = existencia - ? WHERE id = ?;");
 
foreach ($_SESSION["carrito"] as $producto) {
	$total += $producto->total;
	$sentencia->execute([$producto->id,$producto->precioVenta, $idVenta, $producto->cantidad,$producto->descuento]);
